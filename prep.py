...
def save_dataset(args):
    ...
    for p_ind, patient in enumerate(patients_list):
        patient_input_path = os.path.join(args.data_path, patient, "LDCT")
        patient_target_path = os.path.join(args.data_path, patient, "NDCT")
        ...
        for path_ in [patient_input_path, patient_target_path]:
            ...
            for pi in range(len(full_pixels)):
                io = 'input' if 'LDCT' in path_ else 'target'
                f = normalize_(full_pixels[pi], args.norm_range_min, args.norm_range_max)
                f_name = '{}_{:0>4d}_{}.npy'.format(patient, pi, io)
...
def load_scan(path):
    ...
    slices = [pydicom.read_file(os.path.join(path, s)) for s in sorted(os.listdir(path))]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='D:\\cmh\\medical_image\\xiehe_part2\\ori_data\\')
    parser.add_argument('--save_path', type=str, default='./xiehe_npy/')
